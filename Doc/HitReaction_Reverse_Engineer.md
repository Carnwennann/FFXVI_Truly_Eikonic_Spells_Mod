...
### Hooking Physics Update (FUN_14024cc14)

The physics update function can be hooked to modify knockback behavior in real-time.
Physics parameters are stored at `reactionHandler[0xe]` (offset +0x70 from handler base).

```csharp
// Delegate for physics update hook
public unsafe delegate void PhysicsUpdateDelegate(long reactionHandler);

// In hook implementation:
private unsafe void PhysicsUpdateImpl(long reactionHandler)
{
    // Get physics params pointer from handler[0xe]
    long* handlerArray = (long*)reactionHandler;
    long physicsParams = handlerArray[0xe];  // +0x70
    
    if (physicsParams != 0)
    {
        // Modify physics parameters as percentages
        float* sideways = (float*)(physicsParams + 0x18);  // Base velocity
        float* duration = (float*)(physicsParams + 0x08);  // Duration
        float* vertical = (float*)(physicsParams + 0x1c);  // Vertical/deceleration
        float* totalDur = (float*)(physicsParams + 0x3c);  // Total duration
        
        // Example: Double horizontal knockback
        *sideways *= 2.0f;
        
        // Example: Increase launch height
        *vertical *= 1.5f;
    }
    
    // Call original function
    _physicsUpdate.OriginalFunction(reactionHandler);
}
```

#### Physics Parameter Offsets (from physicsParams pointer)

| Offset | Type | Name | Description |
|--------|------|------|-------------|
| +0x08 | float | Duration | Time/duration for calculation |
| +0x18 | float | BaseVelocity | Horizontal knockback speed |
| +0x1c | float | Deceleration | Velocity decay / vertical factor |
| +0x28 | float | AnimationFrame | Animation timing data |
| +0x3c | float | TotalDuration | Total reaction duration |
| +0x52 | byte | GroundedFlag | Is enemy grounded? |
| +0x80 | float | ScaleFactor | Overall physics multiplier |

**Note:** These offsets were identified from Ghidra pseudocode analysis and may need
verification through testing. The actual mapping between SystemMove table columns
and these runtime parameters is:

- `SidewaysMovement` → likely at +0x18
- `SidewaysDuration` → likely at +0x08
- `VerticalPush` → likely affects +0x1c
- `FallGravity` → needs verification
- `DownwardMomentum` → likely affects +0x80 or separate offset
...
- **2024-12-14**: Initial documentation of reaction system
  ...
  - Added physics hook implementation example
